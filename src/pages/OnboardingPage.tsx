...
export function OnboardingPage({onComplete}:{onComplete:(profile:FounderProfile)=>void}){
 ...
 return <main className="onboarding-page"><section className="onboarding-shell phase7-onboarding"><header><span className="onboarding-logo">S</span><div><span className="eyebrow">SMALLBIZ AGENT · QUICK SETUP</span><h1>Meet the AI team for your business.</h1><p>Answer a few simple questions and we’ll put the most useful help first.</p></div></header>
  ...
  <form onSubmit={event=>{event.preventDefault();step===5?finish():setStep(value=>value+1)}}>
   ...
   {step===4&&<fieldset><legend>How should your AI team sound?</legend><div className="setup-options compact">{tones.map(tone=><label key={tone} className={setup.preferredTone===tone?'selected':''}><input type="radio" name="tone" checked={setup.preferredTone===tone} onChange={()=>set('preferredTone',tone)}/><span>{tone}</span></label>)}</div></fieldset>}
   ...
   <div className="onboarding-actions">{step>0?<button type="button" onClick={()=>setStep(value=>value-1)}>Back</button>:<button type="button" className="text-button" onClick={finish}>Skip setup</button>}<button className="primary" disabled={!canContinue} type="submit">{step===5?'Meet my AI team':'Next'}</button></div>
