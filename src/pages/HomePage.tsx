...
export function HomePage({ profile, founderProfile, workflows, reports, approvals, run, go, submitPrompt, promptResult }:Props) {
 ...
 return <div className="page home-page simple-home">
  <header className="welcome"><p>{new Date().toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long'})}</p><h1>Hi {founderProfile.name}, what can we help with?</h1><span>{profile.businessName} · {profile.currentGoal}</span></header>
  ...
  <section><div className="simple-section-title"><h2>Popular ways I can help</h2><button onClick={()=>go('workflows')}>See everything</button></div><div className="simple-quick-grid">{quick.map((item,i)=><button key={item.label} onClick={()=>useQuick(item)}><span>{['☀','♡','✎','⌁','▣','⌕'][i]}</span><b>{item.label}</b></button>)}</div></section>
  ...
  <section className="calm-panel priorities-panel"><div className="simple-section-title"><h2>Today’s priorities</h2><Badge tone="live">For {profile.businessName}</Badge></div>{founderProfile.currentPriorities.map((p,i)=><div className="priority" key={p}><span>{i+1}</span><div><b>{p}</b><small>{i===0?`Suggested next: ${recommended[0]?.name||'Plan my day'}`:profile.biggestChallenge}</small></div></div>)}</section>
 </div>
}
