...
export type PageId = 'home'|'agents'|'workflows'|'approvals'|'reports'|'settings'
const nav: {id:PageId; label:string; icon:string}[] = [
  {id:'home',label:'Command',icon:'⌂'},{id:'agents',label:'Agents',icon:'◎'},{id:'workflows',label:'Workflows',icon:'↗'},
...
export function Shell({ page, setPage, pending, children }:{page:PageId; setPage:(p:PageId)=>void; pending:number; children:ReactNode}) {
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="local-mode"><span className="pulse"/><div><b>Local simulation</b><small>No external actions</small></div></div>
    </aside>
    <main>{children}</main>
