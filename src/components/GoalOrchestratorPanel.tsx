...
import type { GoalOrchestratorBundle, ProjectWorkspaceProject } from '../types'
...
export function GoalOrchestratorPanel({projects,runGoal,openReports,openApprovals}:{projects:ProjectWorkspaceProject[];runGoal:(input:string,projectId?:string)=>GoalOrchestratorBundle|null;openReports:()=>void;openApprovals:()=>void}){
 const [input,setInput]=useState(''); const [projectId,setProjectId]=useState(projects[0]?.id||''); const [bundle,setBundle]=useState<GoalOrchestratorBundle|null>(null)
 const build=()=>{const result=runGoal(input,projectId||undefined); if(result)setBundle(result)}
