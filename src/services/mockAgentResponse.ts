...
import type { AgentOutput, Approval, BusinessProfile, FounderProfile, MockAgentResult, PermissionLevel, RiskLevel, SkillGapReport } from '../types'
...
export function generateMockAgentResponse(prompt:string, preferredAgentId?:string, profile?:BusinessProfile,founder?:FounderProfile):MockAgentResult {
  ...
  const now=new Date().toISOString()
  const context=profile?`BUSINESS CONTEXT USED\nBusiness: ${profile.businessName}\nStage: ${founder?.businessStage||profile.stage}\nIndustry: ${profile.industry}\nProduct/service: ${profile.productOrService}\nTarget customer: ${profile.targetCustomer}\nMain projects: ${founder?.mainProjects.join(', ')||profile.businessName}\nCurrent priorities: ${founder?.currentPriorities.join('; ')||profile.currentGoal}\nBudget: ${profile.budgetLevel}\nPreferred tone: ${founder?.preferredTone||profile.preferredTone}\nRisk preference: ${founder?.riskPreference||profile.riskComfort}\nDefault permission: Level ${founder?.defaultPermissionLevel??1}\n\n`:''
